...
namespace VaccinationScheduling.Offline
{
    ...
    {
        public static Solver CreateSolver(int jobCount, int machineCount, int maxTime, int g,
            (int r1, int d1)[] firstFeasibleIntervals, int[] patientDelays, int[] secondFeasibleIntervalLengths, int p1,
            int p2)
        {
            ...
            for (int i = 0; i < jobCount; i++)
            {
                for (int j = 0; j < 2; j++)
                {
                    for (int m = 0; m < machineCount; m++)
                    {
                        ...
                        {
                            J[i, j, m, t] = solver.MakeIntVar(0, 1, $"J_{i}_{j}_{m}_{t}");
                        }
                    }
                }
            }
            ...
            for (int i = 0; i < jobCount; i++)
            {
                for (int t = 0; t < maxTime; t++)
                {
                    P1[i, t] = solver.MakeIntVar(0, 1, $"P1_{i}_{t}");
                }
            }
            ...
            for (int i = 0; i < jobCount; i++)
            {
                for (int t = 0; t < maxTime; t++)
                {
                    P2[i, t] = solver.MakeIntVar(0, 1, $"P2_{i}_{t}");
                }
            }
            ...
            {
                M[k] = solver.MakeIntVar(0, double.MaxValue, $"M_{k}");
            }
            ...
            for (int i = 0; i < jobCount; i++)
            {
                Constraint constraint = solver.MakeConstraint(2, 2, "2 Jabs for every patient");
                for (int m = 0; m < machineCount; m++)
                {
                    for (int t = 0; t < maxTime; t++)
                    {
            ...
            for (int i = 0; i < jobCount; i++)
            {
                for (int m = 0; m < machineCount; m++)
                {
                    for (int t = 0; t < maxTime - p1; t++)
                    {
                        ...
                        for (int t1 = 0; t < t1 && t1 < t + p1; t1++)
                        {
                            constraint.SetCoefficient(J[i, 1, m, t1], 1);
                        }
                    }
                }
            }
            ...
            for (int i = 0; i < jobCount; i++)
            {
                for (int t = 0; t < maxTime; t++)
                {
                    Constraint constraint = solver.MakeConstraint(firstFeasibleIntervals[i].r1, double.MaxValue,
            ...
            for (int i = 0; i < jobCount; i++)
            {
                for (int t = 0; t < maxTime; t++)
                {
                    Constraint constraint = solver.MakeConstraint(0, firstFeasibleIntervals[i].d1 - p1,
            ...
            for (int i = 0; i < jobCount; i++)
            {
                for (int t = 0; t < maxTime; t++)
                {
                    Constraint constraint = solver.MakeConstraint(p1 + g + patientDelays[i], double.MaxValue,
            ...
            for (int i = 0; i < jobCount; i++)
            {
                for (int t = 0; t < maxTime; t++)
                {
                    Constraint constraint = solver.MakeConstraint(0, p1 + g + patientDelays[i] + secondFeasibleIntervalLengths[i] - p2,
